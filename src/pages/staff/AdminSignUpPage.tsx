...
export default function AdminSignUpPage() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    const { success, error: apiError } = await signUpAdmin(email, password, fullName, adminKey);
    setLoading(false);

    if (!success) {
      setError(apiError || 'Admin sign up failed. Please try again.');
      return;
    }
    ...
    <div className="min-h-screen bg-background flex flex-col justify-center items-center px-4 py-12 relative overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md relative z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground mt-2">Create an administrative account.</p>
        </div>
        ...
        <div className="glass-card p-8 rounded-3xl">
          <form onSubmit={handleSignUp} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-muted-foreground mb-1.5">Admin Registration Key</label>
              <input 
                type="password" 
            ...
            <div className="pt-2">
              ...
              >
                {loading ? <Loader2 size={18} className="animate-spin" /> : <>Create Admin Account <ArrowRight size={18} /></>}
              </button>
            </div>
          </form>
